...
import { 
  ...
  Trash2, Plus, LogOut, Upload, Link as LinkIcon, AlertTriangle, Key, Save
} from 'lucide-react';
...
const AdminPanel: React.FC<AdminPanelProps> = ({ onLogout }) => {
  ...
  const handleAddMockup = (e: React.FormEvent) => {
    ...
    setMockups(addMockup(newMockupCat as any, newMockupUrl));
    setNewMockupUrl('');
  };
  ...
  const handleAddTexture = (e: React.FormEvent) => {
    ...
    setTextures(addTexture(newTextureName, newTextureUrl));
    setNewTextureName('');
    setNewTextureUrl('');
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
               {styles.map(style => (
                 <div key={style} className="flex items-center justify-between p-4 bg-zinc-900 border border-zinc-800 rounded-lg group">
                    ...
                    <button onClick={() => setStyles(removeStyle(style))} className="text-zinc-600 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Trash2 size={16} />
                    </button>
                 </div>
               ))}
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                     {urls.map((url: string, i: number) => (
                       <div key={i} className="aspect-square rounded-lg border border-zinc-800 overflow-hidden relative group">
                          ...
                          <button onClick={() => setMockups(removeMockup(cat as any, url))} className="absolute top-2 right-2 p-2 bg-black/50 text-white rounded-full opacity-0 group-hover:opacity-100 hover:bg-red-600 transition-all">
                             <Trash2 size={14} />
                          </button>
                       </div>
                     ))}
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                 {textures.map((t, i) => (
                    <div key={i} className="aspect-square bg-zinc-900 border border-zinc-800 rounded-lg p-3 flex flex-col items-center gap-3 relative group">
                        ...
                        <button onClick={() => setTextures(removeTexture(t.url))} className="absolute top-2 right-2 p-2 bg-black/50 text-white rounded-full opacity-0 group-hover:opacity-100 hover:bg-red-600 transition-all">
                           <Trash2 size={14} />
                        </button>
                    </div>
                 ))}
