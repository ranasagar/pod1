...
const generateHuggingFace = async (prompt: string, style: string, refImageBase64?: string): Promise<string> => {
    const apiKey = getKeys().huggingface;
...
// --- GEMINI PROVIDER ---
const generateGemini = async (prompt: string, style: string, refImageBase64?: string): Promise<string> => {
...
const generateStability = async (prompt: string, style: string, refImageBase64?: string): Promise<string> => {
  const apiKey = getKeys().stability;
...
const generateOpenAI = async (prompt: string, style: string): Promise<string> => {
...
export const generateDesignWithFallback = async (
  ...
  if (preferredModel !== 'auto') {
    let result = null;
    if (preferredModel === 'gemini') result = await tryProvider('Gemini', () => generateGemini(prompt, style, refImageBase64));
    if (preferredModel === 'stability') result = await tryProvider('Stability', () => generateStability(prompt, style, refImageBase64));
    if (preferredModel === 'dalle') result = await tryProvider('DALL-E', () => generateOpenAI(prompt, style));
    if (preferredModel === 'huggingface') result = await tryProvider('Hugging Face (SDXL)', () => generateHuggingFace(prompt, style, refImageBase64));
  ...
  if (keys.gemini) {
    const result = await tryProvider('Gemini', () => generateGemini(prompt, style, refImageBase64));
    if (result) return result;
  }
  
  if (keys.huggingface) {
    const result = await tryProvider('Hugging Face', () => generateHuggingFace(prompt, style, refImageBase64));
    if (result) return result;
  }

  if (keys.stability) {
    const result = await tryProvider('Stability', () => generateStability(prompt, style, refImageBase64));
    if (result) return result;
  }
  
  if (keys.openai) {
    const result = await tryProvider('DALL-E', () => generateOpenAI(prompt, style));
    if (result) return result;
  }
